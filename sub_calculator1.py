...
def sub_calculator():
    ...
    try:
        ...
        while True:
            ...
            #checking octets
            octet_list = ip.split('.')
            if (len(octet_list) == 4) and (1 <= int(octet_list[0]) <= 223) and (int(octet_list[0]) != 127) and (int(octet_list[0]) != 169
                or int(octet_list[1]) != 254) and (0 <= int(octet_list[1]) <= 255 and 0 <= int(octet_list[2]) <= 255 and 0 <= int(octet_list[3]) <= 255):
                break
            else:
        ...
        #Verifying the Subnet mask 
        valid_subnets = [0, 128, 192, 224, 240, 248, 252, 254, 255]
        while True:
            subnet_mask = input("Enter the subnet mask :")
    
            octet_list = subnet_mask.split('.')

            if (len(octet_list) == 4) and (int(octet_list[0]) in valid_subnets) and (int(octet_list[1]) in valid_subnets) and (int(octet_list[2]
                ) in valid_subnets) and (int(octet_list[3]) in valid_subnets) and (int(octet_list[0])) >= (int(octet_list[1])) >= (int(octet_list[2])
                ) >= (int(octet_list[3])):
                break
